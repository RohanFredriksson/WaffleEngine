...
void Title_Init(Scene* s) {
    ...
    Transform* transformArmaan = TransformPool_Add((vec2){ 0.0f, -1.0f }, (vec2){ 1, 1 }, 0);
    ...
    Transform* transformAinsley = TransformPool_Add((vec2){ -2.9f, 3.0f }, (vec2){ 1, 1 }, 0);

    Component* rigidbodyAinsley = Rigidbody_Init(transformAinsley);
    ((Rigidbody*) rigidbodyAinsley->data)->velocity[0] = 0.0f;
    ((Rigidbody*) rigidbodyAinsley->data)->velocity[1] = 2.0f;
    ((Rigidbody*) rigidbodyAinsley->data)->cor = 0.5f;
    Component* colliderAinsley = Box_Init((vec2) {1.0f, 1.0f}, rigidbodyAinsley);
