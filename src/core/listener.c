...
void MouseListener_CalcOrthoX() {
    ...
    MouseListener_WorldX = camera->position[0] + (currentX / 2.0f) * (camera->projectionSize[0] / camera->zoom);
...
void MouseListener_CalcOrthoY() {
    ...
    Camera* camera =  &(Window_GetScene()->camera);
    MouseListener_WorldY = camera->position[1] - (currentY / 2.0f) * (camera->projectionSize[1] / camera->zoom);
